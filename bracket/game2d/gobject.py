...
def is_color(c):
    ...
    :type c:  any
    """
    from introcs import RGB, HSV, is_tkcolor, is_webcolor
    if type(c) in [RGB, HSV]:
        return True
    ...
    return type(c) == str and (is_tkcolor(c) or is_webcolor(c))
...
class GObject(object):
    ...
    def linecolor(self,value):
        ...
        elif type(value) in [RGB, HSV]:
            value = value.glColor()
        elif type(value) == str:
            if value[0] == '#':
                value = RGB.CreateWebColor(value).glColor()
            else:
                value = RGB.CreateName(value).glColor()
