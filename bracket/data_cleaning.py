...
all_games = data_collection.all_games
winners = data_collection.winners
    ...
    for j in range(len(all_games[0])):
        if('UC' in all_games[i][j]):
            if(all_games[i][j] == 'UCSB'):
                ...
                if(pos != -1):
                    all_games[i][j] = all_games[i][j][:pos] + ' ' + all_games[i][j][pos+1:]  
        elif('cal state' in all_games[i][j].lower()):
            pos = all_games[i][j].lower().find('state')
            all_games[i][j] = all_games[i][j][:pos] + 'St. ' + all_games[i][j][pos + 6:]
...
for i in range(len(mm_games)):
    for j in range(len(mm_games[0])):
        if('UC' in mm_games[i][j]):
            if(mm_games[i][j] == 'UCSB'):
                mm_games[i][j] = 'UC Santa Barbara'
            elif(mm_games[i][j] == "UConn"):
                mm_games[i][j] = 'Connecticut'
            else:
                pos = mm_games[i][j].find('-')
                if(pos != -1):
                    mm_games[i][j] = mm_games[i][j][:pos] + ' ' + mm_games[i][j][pos+1:]  
        elif('cal state' in mm_games[i][j].lower()):
            pos = mm_games[i][j].lower().find('state')
            mm_games[i][j] = mm_games[i][j][:pos] + 'St. ' + mm_games[i][j][pos + 6:]
        elif('state' in mm_games[i][j].lower()):
            if(mm_games[i][j] == 'NC State'):
                mm_games[i][j] = 'North Carolina St.'
            elif(mm_games[i][j] == 'Bowling Green State'):
                mm_games[i][j] = 'Bowling Green'
            elif(mm_games[i][j] != "USC Upstate"):
                mm_games[i][j] = mm_games[i][j][:-3] + '.'
        elif(mm_games[i][j] in name_mappings.keys()):
            mm_games[i][j] = name_mappings[mm_games[i][j]]
...
for i in range(len(mm_winners)):
    if('UC' in mm_winners[i]):
        if(mm_winners[i] == 'UCSB'):
            mm_winners[i] = 'UC Santa Barbara'
        elif(mm_winners[i] == "UConn"):
            mm_winners[i] = 'Connecticut'
        else:
            pos = mm_winners[i].find('-')
            if(pos != -1):
                mm_winners[i] = mm_winners[i][:pos] + ' ' + mm_winners[i][pos+1:]  
    elif('cal state' in mm_winners[i].lower()):
        pos = mm_winners[i].lower().find('state')
        mm_winners[i] = mm_winners[i][:pos] + 'St. ' + mm_winners[i][pos + 6:]
    elif('state' in mm_winners[i].lower()):
        if(mm_winners[i] == 'NC State'):
            mm_winners[i] = 'North Carolina St.'
        elif(mm_winners[i] == 'Bowling Green State'):
            mm_winners[i] = 'Bowling Green'
        elif(mm_winners[i] != "USC Upstate"):
            mm_winners[i] = mm_winners[i][:-3] + '.'
    elif(mm_winners[i] in name_mappings.keys()):
        mm_winners[i] = name_mappings[mm_winners[i]]
...
with open('data/all_games_19.csv', 'w', newline = '') as csvfile:
    ...
    for i in range(len(games)):
        writer.writerow([games['Away Team'][i], games['Home Team'][i], games['Winner'][i]])

with open('data/mm_games_19.csv', 'w', newline = '') as csvfile:
    writer = csv.writer(csvfile, delimiter = ',')
    writer.writerow(['Away Team', 'Home Team', 'Winner'])
    for i in range(len(mm_games)):
        writer.writerow([mm_games['Away Team'][i], mm_games['Home Team'][i], mm_games['Winner'][i]])
